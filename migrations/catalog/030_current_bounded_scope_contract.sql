-- 030_current_bounded_scope_contract.sql
--
-- 部署目標與已發布 bounded snapshot 分離。singleton 空表代表尚未啟用；
-- 只有 run scope 與 desired canonical triple 完全相同時才是 current。

SET SESSION lock_wait_timeout = 30;
SET SESSION innodb_lock_wait_timeout = 30;

DROP PROCEDURE IF EXISTS assert_partsouq_030_preflight;
DELIMITER //
CREATE PROCEDURE assert_partsouq_030_preflight()
BEGIN
  IF DATABASE() IS NULL OR (
    ...
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'crawl_runs'
      AND COLUMN_NAME IN ('scope_brand','scope_model','scope_vehicle_year_floor')
  ) <> 3 OR NOT EXISTS (
    SELECT 1 FROM information_schema.VIEWS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'v_current_catalog_parts'
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'migration 030: apply catalog migrations through 029 first';
  END IF;
  IF EXISTS (SELECT 1 FROM crawl_runs WHERE status = 'running')
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'migration 030: running catalog jobs exist; stop writers first';
  END IF;
END//
DELIMITER ;
CALL assert_partsouq_030_preflight();
DROP PROCEDURE assert_partsouq_030_preflight;
...
DROP PROCEDURE IF EXISTS assert_partsouq_030_output;
DELIMITER //
CREATE PROCEDURE assert_partsouq_030_output()
BEGIN
  IF (
    SELECT COUNT(*) FROM information_schema.COLUMNS
    ...
    WHERE CONSTRAINT_SCHEMA = DATABASE()
      ...
      AND CONSTRAINT_NAME IN (
        'PRIMARY',
        'chk_catalog_desired_scope_singleton',
        'chk_catalog_desired_scope_canonical'
      )
  ) <> 3 OR NOT EXISTS (
    SELECT 1
    FROM information_schema.CHECK_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE()
      AND CONSTRAINT_NAME = 'chk_catalog_desired_scope_singleton'
      AND LOCATE('singleton_id', LOWER(CHECK_CLAUSE)) > 0
      AND LOCATE('= 1', LOWER(CHECK_CLAUSE)) > 0
  ) OR NOT EXISTS (
    SELECT 1
    FROM information_schema.CHECK_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE()
      AND CONSTRAINT_NAME = 'chk_catalog_desired_scope_canonical'
      AND LOCATE('scope_brand', LOWER(CHECK_CLAUSE)) > 0
      AND LOCATE('scope_model', LOWER(CHECK_CLAUSE)) > 0
      AND LOCATE('lower', LOWER(CHECK_CLAUSE)) > 0
      AND LOCATE('1886', LOWER(CHECK_CLAUSE)) > 0
      AND LOCATE('2100', LOWER(CHECK_CLAUSE)) > 0
  ) OR NOT EXISTS (
    SELECT 1 FROM information_schema.VIEWS
    WHERE TABLE_SCHEMA = DATABASE()
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'migration 030: output contract mismatch';
  END IF;
END//
DELIMITER ;
CALL assert_partsouq_030_output();
DROP PROCEDURE assert_partsouq_030_output;
