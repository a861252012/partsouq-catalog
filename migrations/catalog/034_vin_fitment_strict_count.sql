-- 034_vin_fitment_strict_count.sql
--
-- v_vin_part_fitments 原本為每一個零件列重跑 strict candidate COUNT。
-- 正式 10,000 筆 snapshot 會形成 N×N 掃描。改為依 mapping 預先計數一次，
-- 不改變 exact／ambiguous／manual 的判定規則。

SET SESSION lock_wait_timeout = 30;
SET SESSION innodb_lock_wait_timeout = 30;

DROP PROCEDURE IF EXISTS assert_partsouq_034_preflight;
DELIMITER //
CREATE PROCEDURE assert_partsouq_034_preflight()
BEGIN
  IF DATABASE() IS NULL OR (
    SELECT COUNT(*) FROM information_schema.TABLES
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_TYPE = 'BASE TABLE'
      AND TABLE_NAME IN ('admin_vehicle_mappings', 'nhtsa_vin_decodes')
  ) <> 2 OR NOT EXISTS (
    SELECT 1 FROM information_schema.VIEWS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'v_current_catalog_parts'
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'migration 034: apply catalog migrations through 033 first';
  END IF;
END//
DELIMITER ;
CALL assert_partsouq_034_preflight();
DROP PROCEDURE assert_partsouq_034_preflight;
...
DROP PROCEDURE IF EXISTS assert_partsouq_034_output;
DELIMITER //
CREATE PROCEDURE assert_partsouq_034_output()
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.VIEWS
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'migration 034: VIN fitment strict count view is incomplete';
  END IF;
END//
DELIMITER ;
CALL assert_partsouq_034_output();
DROP PROCEDURE assert_partsouq_034_output;
