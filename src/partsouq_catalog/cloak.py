...
def login_to_partsouq() -> bool:
    ...
    with _process_refresh_lock() as acquired:
        ...
        script = (
            ...
            "        # 填寫登入表單（依站方欄位調整選擇器）\n"
            "        await page.fill('input[name=\"YumUserLogin[username]\"]', EMAIL)\n"
            "        await page.fill('input[name=\"YumUserLogin[password]\"]', PASSWORD)\n"
            "        await asyncio.sleep(3)  # 等 Cloudflare Turnstile 自動過關\n"
            "        await page.click('input[name=yt1]')\n"
