...
-- 039 在此之上加入切換閘：合格的全量 archive 出現時，正式 current
-- snapshot 改讀 published_parts（見下方 full_ready／full_snapshot）。
...
-- 039 之後 compatibility 讀者（v_parts 等）改讀切換後的 current snapshot；
-- direct access to published_parts 仍不得繞過 full_ready 閘。
