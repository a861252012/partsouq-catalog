...
def _downgrade_nhtsa_024_schema(cursor: DictCursor) -> None:
    ...
    cursor.execute("DELETE FROM catalog_schema_ledger WHERE version >= 26")
...
def test_forward_cleanup_drops_only_exact_superseded_routines(
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "DELETE FROM catalog_schema_ledger WHERE version IN (18,19,20,21,22,23,24,25,26)"
            )
            cursor.execute(f"CREATE PROCEDURE {exact_name}() SELECT 1")
            cursor.execute(f"CREATE PROCEDURE {near_name}() SELECT 1")

        assert runner.apply() == (18, 19, 20, 21, 22, 23, 24, 25, 26)
        runner.check()
...
def test_migration_022_rebuilds_index_and_rejects_only_invalid_bounded_runs(
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM catalog_schema_ledger WHERE version IN (22,23,24,25,26)")
        ...
        assert runner.apply() == (22, 23, 24, 25, 26)
        runner.check()
        assert runner.apply() == ()
...
def test_migration_020_backfills_and_rejects_legacy_verified_artifact(
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "DELETE FROM catalog_schema_ledger WHERE version IN (20,21,22,23,24,25,26)"
            )

        assert runner.apply() == (20, 21, 22, 23, 24, 25, 26)
        runner.check()
        ...
        with pytest.raises(MigrationError, match="retry that exact version"):
            runner.apply()
        assert runner.apply(retry_version=20) == (20, 21, 22, 23, 24, 25, 26)
        runner.check()
...
def test_migration_023_creates_http_diagnostics_from_missing_table(
    ...
    try:
        ...
        assert runner.apply() == (23, 24, 25, 26)
        runner.check()
        with connection.cursor() as cursor:
            cursor.execute(
...
def test_migration_023_exact_postflight_marks_dirty_before_finish_and_can_retry(
    ...
    try:
        ...
        assert runner.apply(retry_version=23) == (23, 24, 25, 26)
        runner.check()
        with connection.cursor() as cursor:
            cursor.execute("SELECT state,attempt_count FROM catalog_schema_ledger WHERE version=23")
...
def test_migration_024_upgrades_legacy_nhtsa_schema_and_is_repeatable(
    ...
    try:
        ...
        assert runner.apply() == (24, 25, 26)
        runner.check()

        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM catalog_schema_ledger WHERE version IN (24, 25, 26)")
        assert runner.apply() == (24, 25, 26)
        runner.check()
    finally:
        connection.close()
...
def test_migration_024_recovers_one_unique_cross_second_legacy_nhtsa_run(
    ...
    try:
        ...
        assert runner.apply(recover_stale_nhtsa_daemon_seconds=60) == (24, 25, 26)
        runner.check()
